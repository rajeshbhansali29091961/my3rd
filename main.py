...
CURATED = {
    ...
    "BRITANNIA":"ब्रिटानिया कुंजी","DABUR":"डाबर इंडिया",
...
def build_diamond_chart(positions, lagna_sign, title, chart_size=310):
    """
    Draw a standard North Indian diamond Kundali chart using Flet Canvas.
    Houses progress COUNTER-CLOCKWISE from the top central diamond (House 1).
    """
    W = chart_size
    p = 5  # padding
    ...
    # Inner diamond intersection points
    q = W // 4
    iT = (W // 2, p + q)
    iR = (W - p - q, W // 2)
    iB = (W // 2, W - p - q)
    ...
    # 12 Houses mapped COUNTER-CLOCKWISE to match standard North Indian layout
    HOUSES = {
        1:  [T,  iT, iL, L],   # Top Central Diamond (Lagna)
        2:  [TL, T,  iT],      # Top-Left Triangle
        3:  [TL, L,  iL],      # Left-Top Triangle
        4:  [L,  iL, iB, B],   # Left Central Diamond
        5:  [BL, L,  iL],      # Left-Bottom Triangle
        6:  [BL, B,  iB],      # Bottom-Left Triangle
        7:  [B,  iB, iR, R],   # Bottom Central Diamond
        8:  [BR, B,  iB],      # Bottom-Right Triangle
        9:  [BR, R,  iR],      # Right-Bottom Triangle
        10: [R,  iR, iT, T],   # Right Central Diamond
        11: [TR, R,  iR],      # Right-Top Triangle
        12: [TR, T,  iT],      # Top-Right Triangle
    }
    ...
    for house_num, pts in HOUSES.items():
        ...
        # House sign number (Vedic charts display the sign index, plus 1 to be 1-indexed)
        sign_number_display = str(sign_idx + 1)

        # Draw Sign Number (Centered slightly higher)
        shapes.append(cv.Text(
            x=cx - 6, y=cy - 18,
            text=sign_number_display,
            style=ft.TextStyle(
                size=12,
                color="#B71C1C" if is_lagna else "#0D47A1",
                weight="bold")))

        # Draw Sign Linguistic Abbreviation (English + Hindi text)
        shapes.append(cv.Text(
            x=cx - 14, y=cy - 2,
            text=f"{sign_txt}({hindi_txt})",
            style=ft.TextStyle(
                size=9,
                color="#546E7A",
                weight="normal")))

        # Draw Planets occupying this house
        if planet_txt:
            shapes.append(cv.Text(
                x=cx - 22, y=cy + 12,
                text=planet_txt,
    ...
    # Title card overlaying the exact center cross lines
    shapes.append(cv.Text(
        x=C[0] - 28, y=C[1] - 8,
        text=title,
        style=ft.TextStyle(
...
def main(page: ft.Page):
    try:
        ...
        fld_hindi = make_field("Hindi Name *", "e.g. रिलायंस एन्डस्ट्रीज")
        ...
        astro_screen = ft.Column(visible=False, controls=[
            ...
            ft.Container(content=ft.Text(
                "H1 (Red) = Lagna  |  Houses progress counter-clockwise from top",
