...
def main(page: ft.Page):
    try:
        ...
        def load_list(q=""):
            ...
            list_count_txt.value = f"Showing {len(rows)} stocks" + (f" matching '{q}'" if q else (" (Portfolio only)" if fld_portfolio_only.value else " (first 200)"))
...
Tap "📦 LOAD EXAMPLE RULES" to add a 12-rule starter pack covering the patterns above, then edit/delete individual rules to match your own approach."""
        ...
        confirm_exit_panel = ft.Container(
            content=ft.Column([
                ...
                ft.Text("Are you sure you want to close the app? Any unsaved ent
import csv
import io
import time
import math
    ...
    try:
        ...
        def db_search(q, portfolio_only=False):
            try:
                ...
                if portfolio_only:
                    base += " AND portfolio=1"
                base += " ORDER BY symbol LIMIT 200"
                rows = conn.execute(base, ("%" + q + "%", "%" + q + "%")).fetchall()
                conn.close()
                return rows
            except: return []
        ...
        def set_all_portfolio(on):
            try:
                conn = sqlite3.connect(db_path)
                conn.execute("UPDATE stocks SET portfolio=?", (1 if on else 0,))
                conn.commit(); conn.close()
            except: pass
            ...
            for i, r in enumerate(rows):
                ...
                def make_portfolio_toggle(s):
                    def _on_change(e):
                        set_portfolio(s, e.control.value)
                        set_status(("📌 Added to" if e.control.value else "Removed from") + f" portfolio: {s}", C["green"] if e.control.value else C["orange"])
                        if fld_portfolio_only.value and not e.control.value:
                            load_list(fld_list_search.value.strip().upper())  # re-filter list if it just dropped off the ON-only view
                        else:
                            page.update()
                    return _on_change
        ...
        def do_all_portfolio_on(e):
            set_all_portfolio(True)
            set_status("📌 ALL stocks turned ON in your portfolio.", C["green"])
            load_list(fld_list_search.value.strip().upper())

        def do_all_portfolio_off(e):
            set_all_portfolio(False)
            set_status("All stocks turned OFF (portfolio cleared).", C["orange"])
            load_list(fld_list_search.value.strip().upper())
        ...
        list_screen = ft.Column(visible=False, controls=[
            ...
            ft.Divider(height=4, color=C["divider"]),
            ft.Text("📌 PORTFOLIO — each stock defaults OFF; flip its switch to mark it as yours. The on/off state is saved and stays across app restarts.", size=11, color=C["black_txt"]),
            ft.Row([
                ft.ElevatedButton("☑️ ALL ON", bgcolor=C["green"], color="#FFFFFF", height=42, expand=1, on_click=do_all_portfolio_on),
                ft.ElevatedButton("☐ ALL OFF", bgcolor=C["red"], color="#FFFFFF", height=42, expand=1, on_click=do_all_portfolio_off),
            ]),
            fld_portfolio_only,
            list_count_txt, ft.Divider(height=4, color=C["divider"]), list_rows
        ])
