...
Simportss-checks the Graha's signal (Step 5: Bullish/Bearish/Volatile/Speculative) against the Bandha's directional tendency above, to give one final UP / DOWN / SIDEWAYS / MIXED call:
...
def make_report(asum, tval, ldate):
    ...
    if ldate:
        ...
        vedha_partner = VEDHA_PAIRS.get(today_nak_idx)
        has_vedha = (vedha_partner is not None) and (vedha_partner == birth_nak_idx)
        vedha_line = ("⚠️ VEDHA PRESENT — " + nak + " obstructs " + birth_nak + " → avoid fresh entry today" if has_vedha
                       else "✅ NO VEDHA — " + nak + " and " + birth_nak + " are clear of each other")
    else:
        birth_nak = "N/A"
        vedha_line = "N/A (no listing date on record)"
    S  = "─" * 30
    ...
    return "\n".join([
        ...
        "  Symbolic guess, not a guarantee — verify against real price/volume action.", S,
        "STEP 9: SARVATOBHADRA VEDHA CHECK", "  (Muhurta Shastra — Nakshatra Obstruction)",
        "  Today's Nakshatra   : " + nak,
        "  Stock's Nakshatra   : " + birth_nak + " (from listing date)",
        "  " + vedha_line,
        "  (Vedha = a classical affliction between two nakshatras;", "   treat as an extra caution flag, not a standalone rule)", S2,
        "  Research only. Not SEBI advice.", S2,
    ])
...
SARVATOBHADRA VEDHA CHECK (STEP 9 of the Oracle report)
Vedha means "obstruction" — a classical Muhurta-shastra concept where certain pairs of nakshatras are said to afflict/cancel each other's auspiciousness when they occur together. Step 9 compares today's trading-day nakshatra against the stock's own "birth" nakshatra (derived from its listing date) using the standard Sarvatobhadra Chakra pairing table. If they form a Vedha pair, it's flagged ⚠️ as an extra caution signal for that day; if not, it shows ✅ clear. Like the other steps, this is a symbolic overlay — an additional caution flag to weigh alongside Graha, Bandha, and your own Rules, not a standalone reason to act.
...
Tap "📦 LOAD EXAMPLE RULES" to add a 12-rule starter pack covering the patterns above, then edit/delete individual rules to match your own approach."""
        ...
        NAV_ITEMS = [
            ...
            (ft.Icons.EDIT_NOTE,              "Entry",    "entry",  "#6A1B9A"),
            (ft.Icons.STARS,                  "Kundali",  "astro",  "#EF6C00"),
            (ft.Icons.STORAGE,                "Database", "db",     "#455A64"),
            (ft.Icons.RULE,                   "Rules",    "rules",  "#2E7D32"),
        ]
