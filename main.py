...
def main(page: ft.Page):
    try:
        ...
        oracle_screen = ft.Column(visible=True, controls=[
            ...
            fld_oracle,
            ft.ElevatedButton("🔍  SEARCH AND CALCULATE", bgcolor=C["primary"], color="#FFFFFF", elevation=4, height=52, style=ft.ButtonStyle(text_style=ft.TextStyle(size=17, weight="bold")), on_click=do_oracle),
        ...
        list_screen = ft.Column(visible=False, controls=[
            ...
            ft.Row([
                ft.ElevatedButton("🔍 Search", bgcolor=C["primary"], color="#FFFFFF", elevation=3, height=46, on_click=lambda e: load_list(fld_list_search.value.strip().upper())),
                ft.ElevatedButton("📋 Show All", bgcolor=C["accent"], color="#FFFFFF", elevation=3, height=46, on_click=lambda e: load_list("")),
        ...
        entry_screen = ft.Column(visible=False, controls=[
            ...
            fld_sym, fld_eng, ft.ElevatedButton("🌐 AUTO TRANSLITERATE HINDI", bgcolor=C["accent"], color="#FFFFFF", elevation=3, on_click=do_transliterate),
            fld_hindi, ft.ElevatedButton("👁️ PREVIEW SOUND WEIGHTS", bgcolor=C["secondary"], color="#FFFFFF", elevation=3, on_click=lambda e: (asum:=calc(fld_hindi.value.strip())) and setattr(akshara_preview.content,'value',f"Akshara: {asum[0]}\n{asum[1]}") or setattr(akshara_preview,'visible',True) or page.update()),
            akshara_preview, fld_ldate, fld_series, entry_status,
            ft.Row([
                ft.ElevatedButton("💾 SAVE NEW", bgcolor=C["green"], color="#FFFFFF", elevation=3, on_click=do_save),
                ft.ElevatedButton("🔄 UPDATE", bgcolor=C["primary"], color="#FFFFFF", elevation=3, on_click=do_save),
                ft.ElevatedButton("❌ DELETE", bgcolor=C["red"], color="#FFFFFF", elevation=3, on_click=lambda e: db_delete(fld_sym.value.strip().upper()) and setattr(entry_status,'value',"Deleted!") or page.update()),
                ft.ElevatedButton("🧹 CLEAR", bgcolor=C["hint_txt"], color="#FFFFFF", elevation=3, on_click=lambda e: (setattr(fld_sym,'value',""), setattr(fld_sym,'disabled',False), setattr(fld_eng,'value',""), setattr(fld_hindi,'value',""), setattr(fld_ldate,'value',""), setattr(akshara_preview,'visible',False), page.update())),
            ])
        ])
        ...
        fld_lat  = make_field("Latitude (Decimal)", value="19.0544")
        fld_lon  = make_field("Longitude (Decimal)", value="72.8405")
        
        # Main dynamic layout area for structural separation
        ...
        def do_astro(e):
            try:
                ...
                # Ayanamsa Global Info
                astro_chart_container.controls.append(
                    ft.Container(
                        content=ft.Text("✨ SIDEREAL AYANAMSA (LAHIRI): " + str(round(ay, 4)) + "°", size=14, color=C["primary"], weight="bold"),
                        bgcolor=C["res_bg"], padding=10, border_radius=6
                    )
                )
                
                # FIRST: Draw D1 Rasi Chart inside its own clear box view
                astro_chart_container.controls.append(make_header("📍 D1 RASI CHART", bgcolor=C["primary"]))
                astro_chart_container.controls.append(
                    ft.Container(
                        content=build_diamond_chart(d1_pos, lagna_idx, "D1 RASI"),
                        alignment=ft.alignment.center,
                        padding=10
                    )
                )
                
                # Spacer division line
                astro_chart_container.controls.append(ft.Divider(height=10, color=C["divider"]))
                
                # SECOND: Draw D9 Navamsha Chart strictly after D1 is built
                astro_chart_container.controls.append(make_header("📐 D9 NAVAMSHA CHART", bgcolor=C["secondary"]))
                astro_chart_container.controls.append(
                    ft.Container(
                        content=build_diamond_chart(d9_pos, lagna_d9, "D9 NAVAMSHA"),
                        alignment=ft.alignment.center,
                        padding=10
                    )
                )
        ...
        astro_screen = ft.Column(visible=False, controls=[
            ...
            ft.ElevatedButton("🕉️ GENERATE NORTH INDIAN CHARTS", bgcolor=C["green"], color="#FFFFFF", elevation=4, height=52, style=ft.ButtonStyle(text_style=ft.TextStyle(size=16, weight="bold")), on_click=do_astro),
        ...
        def build_db_thread():
            try:
                ...
                conn.close()
                hide_prg()
                set_status(f"Success! {db_count()} stocks loaded.", C["green"])
            except Exception as ex:
                hide_prg()
        ...
        db_screen = ft.Column(visible=False, controls=[
            ...
            ft.ElevatedButton("⚡ BUILD AUTOMATED DATABASE", bgcolor=C["orange"], color="#FFFFFF", elevation=4, height=54, style=ft.ButtonStyle(text_style=ft.TextStyle(weight="bold")), on_click=lambda e: threading.Thread(target=build_db_thread, daemon=True).start()),
            prg_bar, prg_txt
        ])
